...
{
    /// <summary>
    /// Players Async Controller
    /// </summary>
    [ApiVersion("1.0")]
    [Route("api/[controller]")]
    //[Route("api/v{version:apiVersion}/[controller]")]
    [ApiController]
    public class PlayersAsyncController : ControllerBase
    {
        private readonly PlayersServiceAsync<PlayersViewModel, Player> _playersServiceAsync;
        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="playersServiceAsync"></param>
        public PlayersAsyncController(PlayersServiceAsync<PlayersViewModel, Player> playersServiceAsync)
        {
            _playersServiceAsync = playersServiceAsync;
        }
        ...
        public async Task<ActionResult<object>> SelectAsync()
        {
            try
            {
                var items = await _playersServiceAsync.GetAll();
        ...
        public async Task<ActionResult<object>> SelectByIdAsync(int id)
        {
            try
            {
                var item = await _playersServiceAsync.GetOne(id);
                if (item == null)
                {
                    var toSerialize = new MessageHelpers<PlayersViewModel>()
                    {
        ...
        public async Task<ActionResult<object>> DeleteAsync(int id)
        {
            try
            {
                ...
                int retVal = await _playersServiceAsync.Remove(id);
                if (retVal >= 0)
                {
                    var toSerialize = new MessageHelpers<PlayersViewModel>()
                    {
