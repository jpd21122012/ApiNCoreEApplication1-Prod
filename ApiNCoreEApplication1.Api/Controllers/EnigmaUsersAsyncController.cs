...
{
    /// <summary>
    /// EnigmaUsers Async Controller
    /// </summary>
    [ApiVersion("1.0")]
    [Route("api/[controller]")]
    //[Route("api/v{version:apiVersion}/[controller]")]
    [ApiController]
    public class EnigmaUsersAsyncController : ControllerBase
    {
        private readonly EnigmaUsersServiceAsync<EnigmaUsersViewModel, EnigmaUser> _enigmaUsersServiceAsync;
        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="enigmaUsersServiceAsync"></param>
        public EnigmaUsersAsyncController(EnigmaUsersServiceAsync<EnigmaUsersViewModel, EnigmaUser> enigmaUsersServiceAsync)
        {
            _enigmaUsersServiceAsync = enigmaUsersServiceAsync;
        }
        ...
        public async Task<ActionResult<object>> SelectAsync()
        {
            try
            {
                var items = await _enigmaUsersServiceAsync.GetAll();
        ...
        public async Task<ActionResult<object>> SelectByIdAsync(int id)
        {
            try
            {
                var item = await _enigmaUsersServiceAsync.GetOne(id);
                if (item == null)
                {
                    var toSerialize = new MessageHelpers<EnigmaUsersViewModel>()
                    {
        ...
        public ActionResult<object> Post([FromBody] EnigmaUsersViewModel user)
        {
            ...
            else
            {
                try
                {
                    var id = _enigmaUsersServiceAsync.Add(user);
                    var toSerialize = new MessageHelpers<EnigmaUsersViewModel>()
                    {
                        Status = 200,
                        Data = null
        ...
        [Authorize(Roles = "Administrator")]
        [HttpPut("{id}")]
        public async Task<ActionResult<object>> PutAsync(int id, [FromBody] EnigmaUsersViewModel user)
        {
            ...
            else
            {
                try
                {
                    int retVal = await _enigmaUsersServiceAsync.Update(user);
        ...
        public async Task<ActionResult<object>> DeleteAsync(int id)
        {
            try
            {
                ...
                int retVal = await _enigmaUsersServiceAsync.Remove(id);
                if (retVal >= 0)
                {
                    var toSerialize = new MessageHelpers<EnigmaUsersViewModel>()
                    {
