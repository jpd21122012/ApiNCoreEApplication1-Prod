...
{
    /// <summary>
    /// Tournaments Async Controller
    /// </summary>
    [ApiVersion("1.0")]
    [Route("api/[controller]")]
    //[Route("api/v{version:apiVersion}/[controller]")]
    [ApiController]
    public class TournamentsAsyncController : ControllerBase
    {
        private readonly TournamentsServiceAsync<TournamentsViewModel, Tournament> _tournamentsServiceAsync;
        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="tournamentsServiceAsync"></param>
        public TournamentsAsyncController(TournamentsServiceAsync<TournamentsViewModel, Tournament> tournamentsServiceAsync)
        {
            _tournamentsServiceAsync = tournamentsServiceAsync;
        }
        ...
        public async Task<ActionResult<object>> SelectAsync()
        {
            try
            {
                var items = await _tournamentsServiceAsync.GetAll();
        ...
        public async Task<ActionResult<object>> SelectByIdAsync(int id)
        {
            try
            {
                var item = await _tournamentsServiceAsync.GetOne(id);
                if (item == null)
                {
                    var toSerialize = new MessageHelpers<TournamentsViewModel>()
                    {
        ...
        public ActionResult<object> Post([FromBody] TournamentsViewModel user)
        {
            ...
            else
            {
                try
                {
                    var id = _tournamentsServiceAsync.Add(user);
                    var toSerialize = new MessageHelpers<TournamentsViewModel>()
                    {
                        Status = 200,
                        Data = null
        ...
        [Authorize(Roles = "Administrator")]
        [HttpPut("{id}")]
        public async Task<ActionResult<object>> PutAsync(int id, [FromBody] TournamentsViewModel user)
        {
            ...
            else
            {
                try
                {
                    int retVal = await _tournamentsServiceAsync.Update(user);
        ...
        public async Task<ActionResult<object>> DeleteAsync(int id)
        {
            try
            {
                ...
                int retVal = await _tournamentsServiceAsync.Remove(id);
                if (retVal >= 0)
                {
                    var toSerialize = new MessageHelpers<TournamentsViewModel>()
                    {
