...
{
    /// <summary>
    /// EnigmaUsersType Async Controller
    /// </summary>
    [ApiVersion("1.0")]
    [Route("api/[controller]")]
    //[Route("api/v{version:apiVersion}/[controller]")]
    [ApiController]
    public class EnigmaUsersTypeAsyncController : ControllerBase
    {
        private readonly EnigmaUsersTypeServiceAsync<EnigmaUsersTypeViewModel, EnigmaUsersType> _enigmaUsersTypeServiceAsync;
        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="enigmaUsersTypeServiceAsync"></param>
        public EnigmaUsersTypeAsyncController(EnigmaUsersTypeServiceAsync<EnigmaUsersTypeViewModel, EnigmaUsersType> enigmaUsersTypeServiceAsync)
        {
            _enigmaUsersTypeServiceAsync = enigmaUsersTypeServiceAsync;
        }
        ...
        public async Task<ActionResult<object>> SelectAsync()
        {
            try
            {
                var items = await _enigmaUsersTypeServiceAsync.GetAll();
        ...
        public async Task<ActionResult<object>> SelectByIdAsync(int id)
        {
            try
            {
                var item = await _enigmaUsersTypeServiceAsync.GetOne(id);
                if (item == null)
                {
                    var toSerialize = new MessageHelpers<EnigmaUsersTypeViewModel>()
                    {
        ...
        public ActionResult<object> Post([FromBody] EnigmaUsersTypeViewModel user)
        {
            ...
            else
            {
                try
                {
                    var id = _enigmaUsersTypeServiceAsync.Add(user);
                    var toSerialize = new MessageHelpers<EnigmaUsersTypeViewModel>()
                    {
                        Status = 200,
                        Data = null
        ...
        [Authorize(Roles = "Administrator")]
        [HttpPut("{id}")]
        public async Task<ActionResult<object>> PutAsync(int id, [FromBody] EnigmaUsersTypeViewModel user)
        {
            ...
            else
            {
                try
                {
                    int retVal = await _enigmaUsersTypeServiceAsync.Update(user);
        ...
        public async Task<ActionResult<object>> DeleteAsync(int id)
        {
            try
            {
                ...
                int retVal = await _enigmaUsersTypeServiceAsync.Remove(id);
                if (retVal >= 0)
                {
                    var toSerialize = new MessageHelpers<EnigmaUsersTypeViewModel>()
                    {
