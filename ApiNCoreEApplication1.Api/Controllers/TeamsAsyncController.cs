...
{
    /// <summary>
    /// Teams Async Controller
    /// </summary>
    [ApiVersion("1.0")]
    [Route("api/[controller]")]
    //[Route("api/v{version:apiVersion}/[controller]")]
    [ApiController]
    public class TeamsAsyncController : ControllerBase
    {
        private readonly TeamsServiceAsync<TeamsViewModel, Team> _teamsServiceAsync;
        /// <summary>
        /// Constructor
        /// </summary>
        /// <param name="teamsServiceAsync"></param>
        public TeamsAsyncController(TeamsServiceAsync<TeamsViewModel, Team> teamsServiceAsync)
        {
            _teamsServiceAsync = teamsServiceAsync;
        }
        ...
        public async Task<ActionResult<object>> SelectAsync()
        {
            try
            {
                var items = await _teamsServiceAsync.GetAll();
        ...
        public async Task<ActionResult<object>> SelectByIdAsync(int id)
        {
            try
            {
                var item = await _teamsServiceAsync.GetOne(id);
                if (item == null)
                {
                    var toSerialize = new MessageHelpers<TeamsViewModel>()
                    {
        ...
        public async Task<ActionResult<object>> DeleteAsync(int id)
        {
            try
            {
                ...
                int retVal = await _teamsServiceAsync.Remove(id);
                if (retVal >= 0)
                {
                    var toSerialize = new MessageHelpers<TeamsViewModel>()
                    {
